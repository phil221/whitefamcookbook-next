...
const AddRecipeForm = ({ authors }: { authors: Author[] }) => {
    ...
    <div>
      <form action={action}>
        <div className="flex flex-col gap-2 max-w-screen-sm">
          ...
          <select className="border-solid border border-black p-1 rounded" name="authorName">
            {["--- select an author ---",
              ...authors.map((a) => a.name)].map((text, i) => <option key={text} value={i === 0 ? "" : text}>{text}</option>)}
          </select>
